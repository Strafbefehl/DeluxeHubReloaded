...
public class ConfigHandler {
     ...
     * Prevents wiping the config if it failed to load.
     */
    public void save() {
        if (configuration == null || file == null) return;

        try {
            // If config is empty due to a failed load, don't overwrite
            if (configuration.getKeys(false).isEmpty()) {
                plugin.getLogger().warning("Skipping save for " + name + " because configuration is empty or invalid.");
                return;
            }
